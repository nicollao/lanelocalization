...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indexes, mode = 0):
        ...
        # while 1:
            ...
            for index in indexes:
                ...
                # raw_laneidx = classdefine.simpleclass_to_type(classdefine.type_to_classnum(lane)[1])
                ...
                if mode == 0:
                    ...
                    # new_image = raw_image
                    # new_laneidx = raw_laneidx
                else:
                    new_image, new_laneidx = self.generate_new_image2(raw_image, raw_laneidx, mode)

                X_batch.append(new_image)
                y_batch.append(new_laneidx)
            if myconfig.def_output_type == 1:
                # yield np.array(X_batch), np.array(y_batch)
                return np.array(X_batch), np.array(y_batch)
            else:
                # yield np.array(X_batch), np_utils.to_categorical(np.array(y_batch), self.n_classes)
                return np.array(X_batch), np_utils.to_categorical(np.array(y_batch), self.n_classes)


    def generate_new_image2(image, laneidx=0, mode=1, index=1, top_crop_percent=0.35, bottom_crop_percent=0.1,
                            resize_dim=(myconfig.resize_image_height, myconfig.resize_image_width), do_shear_prob=0.9):

        # apply cannyEdge filter
        if mode == 1:
            image1 = crop(image, top_crop_percent, bottom_crop_percent)
            # mpimg.imsave('./image/cropimg_%d_01.jpg' % index, image1)
            # image2 = contrast_image(image1)
            # mpimg.imsave('./image/cropimg_%d_02.jpg' % index, image2)
            image3 = imagecombine(image1, index)
            # mpimg.imsave('./image/cropimg_%d_03.jpg' % index, image3)
            result = resize(image3, resize_dim)
            # mpimg.imsave('./image/cropimg_%d_04.jpg' % index, result)

        elif mode == 2:

            if len(warp_src) == 0 or len(warp_dst) == 0:
                warp_initial(image, top_crop_percent, bottom_crop_percent)
            image1 = warp_image(image, warp_src, warp_dst)
            # mpimg.imsave('./image/cropimg_%d_01.jpg' % index, image1)
            result = resize(image1, resize_dim)
            # mpimg.imsave('./image/cropimg_%d_04.jpg' % index, result)
        else:
            ...
            image2 = contrast_image(image1)
            # mpimg.imsave('./image/cropimg_%d_02.jpg' % index, image2)
            image3 = imagecombine(image2, index)
            # mpimg.imsave('./image/cropimg_%d_03.jpg' % index, image3)
            result = resize(image3, resize_dim)
            # mpimg.imsave('./image/cropimg_%d_04.jpg' % index, result)
    ...
    def generate_new_image(self, image, laneidx, resize_dim=(myconfig.resize_image_height, myconfig.resize_image_width)):

        top_percent = uniform(0.3, 0.5)
        bottom_percent = uniform(0.0, 0.1)

        image = self.crop(image, top_percent, bottom_percent)
        # image = self.random_gamma(image)
        image = self.resize(image, resize_dim)
        # mpimg.imsave('cropimg01.jpg', image)
        # image = image.astype('float32')
        # mpimg.imsave('cropimg02.jpg', image)
        # image = image / 127.5 - 1
        # mpimg.imsave('cropimg03.jpg', image)
        # print(image.shape, 'train samples')
        return image, laneidx
    ...
    def generate_next_batch(batch_size=64, mode=0, val_data=0):
        ...
        while 1:
            ...
            for img_file, laneidx in images:
                ...
                if mode == 0:
                    new_image, new_laneidx = generate_new_image(raw_image, raw_laneidx)
                else:
                    new_image, new_laneidx = generate_new_image2(raw_image, raw_laneidx, mode)

                X_batch.append(new_image)
                y_batch.append(laneidx)
            if myconfig.def_output_type == 1:
                yield np.array(X_batch), np.array(y_batch)
            else:
